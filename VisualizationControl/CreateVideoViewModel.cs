        ...
        private static readonly Dictionary<VideoDisplayResolution, Tuple<int, int>> resolutions = new Dictionary<VideoDisplayResolution, Tuple<int, int>>()
    {
      ...
      },
      {
                ...
                try
                {
                    ...
                    if (this.worker.IsBusy)
                    {
                        ...
                        Task.Factory.StartNew((Action)(() =>
                        {
                            try
                            {
                                this.cancellationEvent.WaitOne(CancellationTimeout);
                            }
                            catch (Exception ex)
                            {
        ...
        private void ProcessVideo(object sender, DoWorkEventArgs e)
        {
            ...
            HostControlViewModel hostControlViewModel = null;
            Bitmap bitmap = null;
            BitmapData data = null;
            this.dispatcher.Invoke((() =>
            {
                int width = resolutions[this.SelectedResolution].Item1;
                int height = resolutions[this.SelectedResolution].Item2;
                CancellationTokenSource cancellationTokenSource = new CancellationTokenSource();
                try
                {
                    int scale = width == WidthLowDef ? 2 : 1;
                    this.visualizationModel.Initialize(this.hostControlViewModelOriginal.Model.CustomMapProvider, this.hostControlViewModelOriginal.Model.BingMapResources, width * scale, height * scale, 30);
                    hostControlViewModel = new HostControlViewModel(this.visualizationModel, null, this.hostControlViewModelOriginal.BingMapResourceUri, this.hostControlViewModelOriginal.CustomColors);
                    hostControlViewModel.SetTour(this.visualizationModel.CurrentTour);
                    hostControlViewModel.OnExecutePlayTour();
                    bitmap = new Bitmap(width, height);
                    data = bitmap.LockBits(new Rectangle(0, 0, width, height), ImageLockMode.ReadWrite, PixelFormat.Format32bppPArgb);
                    VideoImageSource videoImageSource = new VideoImageSource(hostControlViewModel, data, this.dispatcher, scale);
                    this.encodingStatus = this.session.ConfigureVideoOptions(this.SelectedFileName, TourVideoSession.GetSupportedResolution(width, height), data, 30U);
                    ...
                    else
                    {
                        ...
                        double frameCount = 30.0 * this.visualizationModel.CurrentTour.Duration;
                        ...
                        while (hostControlViewModel.TourPlayer.TourPlayer.IsPlaying && !this.worker.CancellationPending)
                        {
                            ...
                            {
                                VisualizationTraceSource.Current.TraceInformation(
                                    "Fetched Frame# {0} of {1} from engine in {2} ms.", 
                                    processedFrameCount + 1, 
                                    frameCount, 
                                    stopwatch.Elapsed.TotalMilliseconds);
                                stopwatch.Restart();
                                if (processedFrameCount % 90 == 0 && 
                                    (this.fileNotFound = !File.Exists(this.SelectedFileName)) || 
                                    (this.encodingStatus = this.session.AddFrame()) == EncodingStatus.VideoFrameProcessingError)
                                {
                                    if (!task.IsCompleted)
                                ...
                                else
                                {
                                    stopwatch.Stop();
                                    VisualizationTraceSource.Current.TraceInformation(
                                        "Wrote Frame# {0} of {1} to video stream in {2} ms.", 
                                        processedFrameCount + 1, 
                                        frameCount, 
                                        stopwatch.Elapsed.TotalMilliseconds);
                                    int percent = (int)(++processedFrameCount * 100.0 / frameCount);
                                    this.worker.ReportProgress(percent < 100 ? percent : 99);
                                }
                            }
                        }
                ...
                {
                    cancellationTokenSource.Cancel();
                    VisualizationTraceSource.Current.Fail(string.Format("Encountered an error while producing video at resolution {0}x{1}. Exception Message: {2}.", (object)width, (object)height, (object)ex.Message));
                    this.failed = true;
                }
                finally
                {
            ...
            this.dispatcher.Invoke((() =>
            {
                try
                {
                    ...
                    {
                        hostControlViewModel.TourPlayer.ExitTourPlaybackModeCommand.Execute(null);
