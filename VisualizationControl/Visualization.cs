...
namespace Microsoft.Data.Visualization.VisualizationControls
{
  public abstract class Visualization
  {
    ...
    internal VisualizationModel VisualizationModel
    {
      get
      {
        ...
        if (layerManager != null)
          ...
          return (VisualizationModel) null;
      }
    }

    protected bool RefreshDisplayPending { get; set; }

    internal Visualization(Visualization.SerializableVisualization state, LayerDefinition layerDefinition, DataSource dataSource)
    {
    ...
    internal void NotifyQueryCompletion(LayerManager layerManager, bool isVisible, Exception ex)
    {
      lock (this.queryCompletionNotificationList)
      {
        foreach (LayerManager.Settings item_0 in this.queryCompletionNotificationList)
        {
          try
          {
            layerManager.RefreshSettings(isVisible, item_0, ex);
          }
    ...
    internal void PrepareData(bool zoomToData = false, CancellationTokenSource cancellationTokenSource = null, bool forceRequeryIfVisible = false, bool forciblyRefreshDisplay = false, LayerManager.Settings layerManagerSettings = null)
    {
      ...
      try
      {
        ...
        flag = false;
        this.NotifyQueryCompletion(layerManager, isVisible, (Exception) null);
      }
      catch (Exception ex)
      {
        if (flag)
    ...
    internal virtual void Shutdown()
    {
      ...
      this.DataBinding = (DataBinding) null;
      this.LayerDefinition = (LayerDefinition) null;
      if (this.FieldWellDefinition == null)
        return;
      this.FieldWellDefinition.Shutdown();
      this.FieldWellDefinition = (FieldWellDefinition) null;
    }

    protected virtual void SnapState(Visualization.SerializableVisualization state)
    {
      state.Visible = this.Visible;
    }

    protected virtual void SetStateTo(Visualization.SerializableVisualization state)
    {
