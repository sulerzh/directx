...
// Type: Microsoft.Data.Visualization.Engine.Layer
...
namespace Microsoft.Data.Visualization.Engine
{
  public abstract class Layer
  {
    ...
    public float DataDimensionScale
    {
      ...
      set
      {
        if ((double) value == (double) this.dataDimensionScale)
          return;
    ...
    public float FixedDimensionScale
    {
      ...
      set
      {
        if ((double) value == (double) this.fixedDimensionScale)
          return;
        if (this.EngineDispatcher != null)
        {
          this.EngineDispatcher.RunOnRenderThread((RenderThreadMethod) (() =>
    ...
    public double ViewScale
    {
      get
      {
        return (double) this.scaling.ViewScale;
      }
    }
    ...
    protected Coordinates CoordinatesFromLongLatDegrees(double longitude, double latitude)
    {
      ...
      double to_lat_y;
      double to_long_x;
      this.DataInputCustomSpace.TransformSpacesDataToDegrees(latitude, longitude, out to_lat_y, out to_long_x);
      return Coordinates.FromDegrees(to_long_x, to_lat_y);
    }
    ...
    protected float GetLayerClampedValue(double value)
    {
      ...
      if (this.UseLogarithmicClampedValue && value != 0.0)
        value = Math.Max(0.0, 0.0482549424336947 * Math.Log(Math.Abs(value)) + 1.0) * (double) Math.Sign(value);
      if (Math.Abs(value) < 1E-06)
        value = 1E-06 * (double) Math.Sign(value);
      return (float) value;
    }
