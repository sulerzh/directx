...
// Type: Microsoft.Data.Visualization.Engine.InstanceRenderer
  ...
  internal class InstanceRenderer : DisposableResource
  {
    ...
    public bool HasNegativeValues()
    {
      if (!this.UseTextureForNegativeValues)
      {
        ...
        {
          foreach (InstanceBlock item_0 in this.instanceBlocks)
          {
            if (item_0.HasNegativeValues)
              return true;
          }
        }
      }
    ...
    public void SetSelectedItems(IEnumerable<InstanceId> selectedIds)
    {
      ...
      {
        foreach (InstanceBlock item_0 in this.instanceBlocks)
          item_0.ClearFilterInstances();
        foreach (InstanceId item_1 in selectedIds)
        {
          Tuple<uint, uint> local_2 = this.GetInstanceBufferPosition(item_1);
          if (local_2 != null)
            this.instanceBlocks[(int) local_2.Item1].AddFilterInstance((int) local_2.Item2);
        }
      }
    }
    ...
    public void SetAnnotations(IEnumerable<InstanceId> annotationIds)
    {
      ...
      {
        foreach (InstanceBlock item_0 in this.instanceBlocks)
          item_0.ClearAnnotationInstances();
        foreach (InstanceId item_1 in annotationIds)
        {
          Tuple<uint, uint> local_2 = this.GetInstanceBufferPosition(item_1);
          if (local_2 != null)
            this.instanceBlocks[(int) local_2.Item1].AddAnnotationInstance((int) local_2.Item2);
        }
      }
    }
    ...
    private void UpdateIdMap()
    {
      ...
      lock (this.instanceBlocksLock)
      {
        for (int local_0 = 0; local_0 < this.instanceBlocks.Count; ++local_0)
        {
          for (int local_1 = 0; local_1 < this.instanceBlocks[local_0].Count; ++local_1)
          {
            uint local_2 = this.instanceBlocks[local_0].GetInstanceIdAt(local_1).ElementId;
            if (!this.idToBufferPositionMap.ContainsKey(local_2))
              this.idToBufferPositionMap.Add(local_2, new Tuple<uint, uint>((uint) local_0, (uint) local_1));
          }
        }
      }
    }
    ...
    private bool Update(SceneState state)
    {
      ...
      ++this.frameCount;
      bool flag = false;
      if ((double) this.scaling.EaseInScale < 1.0)
        flag = true;
      ...
      this.timeScaling.Update(new DateTime?(this.minVisualTime), new DateTime?(this.maxVisualTime), state);
      ...
      if (this.updateIndex)
      {
        ...
        this.spatialIndex.Traverse((IQuery) this.visualLayer.GetSpatialQuery(state, this.visibleBlocks));
      }
    ...
    private void InitializeTechniques()
    {
      ...
      {
        (IRenderParameter) this.instanceViewParameter,
        (IRenderParameter) this.instanceViewProjParameter,
        (IRenderParameter) this.instanceViewProjDepthOffsetParameter[0],
        (IRenderParameter) this.instanceViewProjDepthOffsetParameter[1],
        (IRenderParameter) this.instanceViewProjDepthOffsetParameter[2],
        (IRenderParameter) this.instanceViewProjDepthOffsetParameter[3],
        (IRenderParameter) this.instanceViewProjDepthOffsetParameter[4],
        (IRenderParameter) this.instanceBasePositionParameter,
        (IRenderParameter) this.variableScaleParameter,
        (IRenderParameter) this.fixedScaleParameter,
        (IRenderParameter) this.fixedDimensionParameter,
        (IRenderParameter) this.heightOffsetParameter,
        (IRenderParameter) this.horizontalSpacingParameter,
        (IRenderParameter) this.viewScaleParameter,
        (IRenderParameter) this.viewScaleUnboundParameter,
        ...
        (IRenderParameter) this.visualTimeFreezeEnableParameter,
        (IRenderParameter) this.applyTextureParameter,
        (IRenderParameter) this.zValuesSignParameter,
        (IRenderParameter) this.alignmentParameter,
        (IRenderParameter) this.maxValueParameter,
        (IRenderParameter) this.annotationAnchorHeightParameter,
        (IRenderParameter) this.opacityParameter
      });
    ...
    public void UpdateDisplayOptions(bool nulls, bool zeros, bool negatives)
    {
      ...
      {
        foreach (InstanceBlock item_0 in this.instanceBlocks)
          item_0.ShowNegatives = this.showNegatives;
      }
    }
        ...
        if (dictionary[queryType])
        {
          ...
          for (int index = 0; index < this.visibleBlocks.Count; ++index)
          {
            ...
            if (queryType != InstanceBlockQueryType.NegativeInstances || instanceBlock.HasNegativeValues)
            {
              ...
              if (queryType == InstanceBlockQueryType.NullInstances && (double) valueOffset > 0.0)
                valueOffset = 0.0001f;
              this.SetScale(queryType);
              float num1 = (float) index * (1.0f / 1000.0f);
              ...
              if (num2 != 0)
              {
                ...
                {
                  float num3 = Math.Min(1f, (float) ((double) (this.lastUpdateTick - this.desaturationChangedTick) / 60.0 / 0.300000011920929));
                ...
                DrawParameters parameters = new DrawParameters()
                {
                  ...
                  InstanceCount = num2,
                  PositionProvider = (IInstancePositionProvider) instanceBlock,
                  QueryType = queryType,
    ...
    {
      float num = (float) ((this.frameCount - 1) % 16000 - 8000);
      if ((double) num == -8000.0)
    ...
    private void UpdateGatherAccumulate(Renderer renderer, SceneState state, DrawMode drawMode, DrawStyle drawStyle)
    {
      ...
      this.gatherAccumulate.Process((IList<GatherAccumulateProcessBlock>) list, renderer, ignoreValues, constantMode);
    }
    ...
    private int QueryInstances(Renderer renderer, SceneState state, DrawStyle drawStyle, DrawMode drawMode, float frameId, bool showOnlyMaxValues, bool useLogScale, float valueOffset, InstanceBlockQueryType queryType, InstanceBlock instanceBlock, out StreamBuffer streamBuffer, out StreamBuffer idStreamBuffer)
    {
      ...
      idStreamBuffer = (StreamBuffer) null;
    ...
    private void UpdateHitTestGatherAccumulate(Renderer renderer, DrawMode drawMode)
    {
      ...
      for (int index = 0; index < this.visibleBlocks.Count; ++index)
        list.Add(this.instanceBlocks[this.visibleBlocks[index]].GetGatherAccumulateBlock());
      this.gatherAccumulate.ProcessHitTest((IList<GatherAccumulateProcessBlock>) list, renderer, (drawMode & DrawMode.PointMarker) > DrawMode.None, (drawMode & DrawMode.ConstantMode) > DrawMode.None);
    }
    ...
    private void SetScale(InstanceBlockQueryType queryType)
    {
      ...
      bool flag = (double) variableScale.X != 0.0;
    ...
    private bool DrawBufferAnnotation(Renderer renderer, SceneState state, DrawParameters parameters, InstanceBlock block)
    {
      ...
      renderer.SetEffect((EffectTechnique) this.annotationTechnique);
      ...
      if (vertexBuffer1 != null)
      {
        ...
        if (parameters.DrawStyle == DrawStyle.Annotation || parameters.DrawStyle == DrawStyle.AnnotationHitTest)
        {
          ...
          {
            InstanceId? annotationInstanceId = block.GetAnnotationInstanceId(index, new DateTime?(state.VisualTimeFreeze ?? state.VisualTime), state);
            if (annotationInstanceId.HasValue)
            {
              ...
              this.annotationTechnique.TextureDimensions = mostRecentTexture == null ? Vector2F.Empty : new Vector2F((float) mostRecentTexture.Width, (float) mostRecentTexture.Height);
    ...
    private bool DrawSelection(Renderer renderer, SceneState state, DrawMode drawMode)
    {
      ...
      bool flag;
      lock (this.instanceBlocksLock)
        flag = this.Draw(renderer, state, drawMode, DrawStyle.Selection, (DepthStencilState) null, (BlendState) null, (RasterizerState) null);
    ...
    public bool DrawAnnotation(Renderer renderer, SceneState state, DrawMode drawMode, bool blockUntilComplete)
    {
      ...
      bool flag;
      lock (this.instanceBlocksLock)
        flag = this.Draw(renderer, state, drawMode, DrawStyle.Annotation, (DepthStencilState) null, (BlendState) null, (RasterizerState) null);
      renderer.Profiler.EndSection();
      return flag;
    }
    ...
    protected override void Dispose(bool disposing)
    {
      ...
      {
        (DisposableResource) this.processingTechnique,
        (DisposableResource) this.annotationTechnique,
        (DisposableResource) this.gatherAccumulate,
        (DisposableResource) this.sharedParameters,
        (DisposableResource) this.instanceStreams
      };
