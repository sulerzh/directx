...
bool DeviceContext::GetData(Asynchronous^ asyncData, IntPtr data, UInt32 dataSize, AsyncGetDataOptions options)
{
	HRESULT hr = CastInterface<ID3D11DeviceContext>()->GetData(asyncData->CastInterface<ID3D11Asynchronous>(), data.ToPointer(), static_cast<UINT>(dataSize), static_cast<UINT>(options));
	if (hr == 0) return true;
