...
void D3DImage11::SetBackBuffer11(IntPtr pResource)
{
...
D3DFORMAT D3DImage11::ConvertDXGIToD3D9Format(DXGI_FORMAT format)
{
	switch (format)
	{
...
HRESULT D3DImage11::GetSharedSurface(HANDLE hSharedHandle, void** ppUnknown, UINT width, UINT height, DXGI_FORMAT format)
{
	...
	* pulls the D3D10/11 resource in the 9Ex device */
	hr = mD3D9Device->CreateTexture(width, height, 1,
...
HRESULT D3DImage11::GetSharedHandle(IUnknown *pUnknown, HANDLE * pHandle)
{
	HRESULT hr = S_OK;
	...
	*pHandle = NULL;
	...
	if (FAILED(hr = pUnknown->QueryInterface(
...
HRESULT D3DImage11::InitD3D9(HWND hWnd)
{
	HRESULT hr;
	...
	hr = mD3D9->CreateDeviceEx(D3DADAPTER_DEFAULT,
		D3DDEVTYPE_HAL,
		hWnd,
